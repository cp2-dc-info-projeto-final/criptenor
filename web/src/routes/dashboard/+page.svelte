<script lang="ts">
  ...
  try {
    ...
    if (response.ok) {
      ...
      let totalCredito=data.totalCredito;
      console.log(totalCredito);
      localStorage.setItem("totalCredito", data.totalCredito);
      return data.totalCredito; // Retorna o valor total de crédito
    } else {
